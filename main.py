...
async def vk_settings_update(request: Request):
    try:
        body = await request.json()
        print(f"VK UPDATE body: {body}")
        s = next(db_session())
        vk = get_vk_settings(s)
        print(f"VK before: api_key='{vk.api_key}' group_id='{vk.selected_group_id}'")
        if "api_key" in body:
            vk.api_key = body["api_key"]
        if "selected_group_id" in body:
            vk.selected_group_id = str(body["selected_group_id"])
        s.commit()
        print(f"VK after: api_key='{vk.api_key}' group_id='{vk.selected_group_id}'")
        s.close()
        return {"ok": True}
    except Exception as e:
        print(f"VK UPDATE ERROR: {e}")
